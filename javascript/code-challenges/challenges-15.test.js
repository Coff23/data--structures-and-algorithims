...
const screenForNames = (arr) => {
  // Solution code here...
  const pattern = /^((Mr|Mrs|Ms|Dr)\. [A-Z][a-z]+)$/;
...
let biggerThanLuke = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].mass && Number(arr[i].mass) > 77) {
      if (result === '') {
        result += arr[i].name;
      } else {
        result += ' - ' + arr[i].name;
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  const checker = (row1, col1, row2, col2, row3, col3) => {
    const first = board[row1][col1];
    if (!first) {
      return false;
    }
    return first === board[row2][col2] && first === board[row3][col3];
  };
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (checker(0, i, 1, i, 2, i)) {
      return true;
    }
  }

  if (checker(0, 0, 1, 1, 2, 2)) {
    return true;
  }
  if (checker(2, 0, 1, 1, 0, 2)) {
    return true;
  }
